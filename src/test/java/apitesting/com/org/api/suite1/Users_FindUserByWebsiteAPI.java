...
public class Users_FindUserByWebsiteAPI {
	...
	public void setUp() throws Exception {
		...
		csvDataForTestCase = util.readData("config\\data.properties", "api.findUserByWebsite.data.filepath",cvsSplitBy);
	}
	@Test
	public void Test_FindUsersByWebsite() {
		att_path1 = csvDataForTestCase.get(1).get(1);
		resp = given().when().get(urlServer + "?website="+att_path1+"");
	...
	@Test
	public void Test_FindUsersByWebsiteNotExisted() {
		att_path2 = csvDataForTestCase.get(2).get(1);
		resp = given().when().get(urlServer + "?website="+att_path2+"");
		statusCode = resp.getStatusCode();
		body = resp.andReturn().body().asString();
		Assert.assertEquals(statusCode, 200);
	...
	@Test
	public void Test_FindUsersByBlankWebsite() {
		resp = given().when().get(urlServer + "?website=");
		statusCode = resp.getStatusCode();
		body = resp.andReturn().body().asString();
		Assert.assertEquals(statusCode, 200);
		Assert.assertEquals(body,"[]");
	...
	@Test
	public void Test_FindUsersByNumberWebsite() {
		att_path3 = csvDataForTestCase.get(2).get(1);
		resp = given().when().get(urlServer + "?website="+att_path3+"");
		statusCode = resp.getStatusCode();
		body = resp.andReturn().body().asString();
		Assert.assertEquals(statusCode, 200);
		Assert.assertEquals(body,"[]");
	...
	@Test
	public void Test_FindUsersByWebsiteWithInvalidURL() {
	...
	@Test
	public void Test_FindUsersByWebsiteWithDifferentParameter() {
