...
public class Users_FindUserByEmailAPI {
	...
	public void setUp() throws Exception {
		...
		csvDataForTestCase = util.readData("config\\data.properties", "api.findUserByEmail.data.filepath", cvsSplitBy);
	}

	@Test
	public void Test_FindUsersByEmail() {
		att_path1 = csvDataForTestCase.get(1).get(1);
		resp = given().when().get(urlServer + "?email=" + att_path1 + "");
	...
	@Test
	public void Test_FindUsersByBlankEmail() {
		resp = given().when().get(urlServer + "?email=");
		statusCode = resp.getStatusCode();
		body = resp.andReturn().body().asString();
		jsonPrint.JsonPrintUserName(parser, body);
		Assert.assertEquals(statusCode, 200);
	...
	@Test
	public void Test_FindUsersByInvalidEmail() {
		att_path2 = csvDataForTestCase.get(2).get(1);
		resp = given().when().get(urlServer + "?email="+att_path2+"");
		statusCode = resp.getStatusCode();
		body = resp.andReturn().body().asString();
		jsonPrint.JsonPrintUserName(parser, body);
		Assert.assertEquals(statusCode, 200);
	...
	@Test
	public void Test_FindUsersByNotExistedEmail() {
		att_path3 = csvDataForTestCase.get(3).get(1);
		resp = given().when().get(urlServer + "?email="+att_path3+"");
		statusCode = resp.getStatusCode();
		body = resp.andReturn().body().asString();
		jsonPrint.JsonPrintUserName(parser, body);
		Assert.assertEquals(statusCode, 200);
	...
	@Test
	public void Test_FindUsersByEmailWithNotValidURL() {
		att_path4 = csvDataForTestCase.get(4).get(1);
		resp = given().when().get(urlServer + "@?email="+att_path4+"");
		statusCode = resp.getStatusCode();
		body = resp.andReturn().body().asString();
		//jsonPrint.JsonPrintUserName(parser, body);
